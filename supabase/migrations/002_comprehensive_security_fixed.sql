...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "System only access to password history" ON password_history;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "No direct OTP access" ON otp_codes;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Enrolled users can read lessons" ON lessons;
